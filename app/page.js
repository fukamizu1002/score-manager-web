...
function standardExamPresets(){
 ...
 ["9月","10月","11月","12月","1月"].forEach(month=>push(`Vもぎ ${month}`,range(2019,2025),["国語","数学","英語","理科","社会"]));
...
function eraYear(year){
 const y=Number(year);if(!Number.isFinite(y))return year||"—";
...
function Exams({cid,data}){
 ...
 return <div className="grid"><div className="card s4"><h2>{editingId?"過去問を編集":"過去問追加"}</h2><form className="form" onSubmit={save}><F l="過去問の種類"><select value={f.category} onChange={e=>chooseCategory(e.target.value)}>{Object.keys(CATEGORY_CONFIG).map(x=><option key={x}>{x}</option>)}</select></F><F l="学校名"><input required value={f.school} onChange={e=>setF({...f,school:e.target.value})}/></F><F l="年度（西暦で入力）" c="f6"><input type="number" value={f.year} onChange={e=>setF({...f,year:e.target.value})}/><small className="muted">表示：{String(f.school).startsWith("Vもぎ")?`${f.year}年度`:eraYear(f.year)}</small></F><F l="科目" c="f6"><select value={f.subject} onChange={e=>setF({...f,subject:e.target.value})}>{CATEGORY_CONFIG[f.category].subjects.map(x=><option key={x}>{x}</option>)}</select></F><F l="満点" c="f6"><input type="number" value={f.max} onChange={e=>setF({...f,max:e.target.value})}/></F><button className="btn primary">{editingId?"更新":"追加"}</button>{editingId&&<button type="button" className="btn ghost" onClick={()=>{setEditingId(null);setF(initial)}}>キャンセル</button>}</form><hr/><h3>定番過去問の一括登録</h3><p className="muted">都立高共通・自校作成校・都立国際・Vもぎ・都立中共同作成・学校別独自問題・区立九段を登録します。</p><button className="btn primary" type="button" disabled={bulkBusy} onClick={bulkAdd}>{bulkBusy?"登録中…":"確定済み過去問を一括登録"}</button>{bulkMessage&&<p>{bulkMessage}</p>}</div><div className="card s8"><h2>過去問一覧</h2><div className="table"><table><thead><tr><th>種類</th><th>学校</th><th>年度</th><th>科目</th><th>満点</th><th>操作</th></tr></thead><tbody>{data.map(e=><tr key={e.id}><td>{e.category||e.type||"未分類"}</td><td>{e.school}</td><td>{examYear(e)}</td><td>{e.subject}</td><td>{e.max}点</td><td><button className="btn ghost" onClick={()=>edit(e)}>編集</button> <button className="btn danger" onClick={()=>confirm("削除しますか？")&&deleteDoc(doc(db,"campuses",cid,"exams",e.id))}>削除</button></td></tr>)}</tbody></table></div></div></div>
}
         ...
         <table>
           ...
           <tbody>
             {rows.slice().reverse().map(r=>{
               let e=exams.find(x=>x.id===r.examId)||{};
               return <tr key={r.id}>
                 <td>{r.date}</td><td>{e.category||e.type||"未分類"}</td><td>{e.school}</td><td>{examYear(e)}</td><td>{e.subject}</td>
                 <td>{r.score}/{e.max}</td>
