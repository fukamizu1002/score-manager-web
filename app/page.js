...
function Entry({cid,students,exams,scores}){
 const blank={date:today(),grade:"",studentId:"",category:"",subject:"",year:"",examId:"",score:"",submitDue:"",submitDate:"",reviewDue:"",reviewDate:"",reviewStatus:"未復習",minutes:"",teacherComment:""};
 const examCategory=e=>e.category||e.type||"";
 const [f,setF]=useState(blank), allowedCategories=categoriesForGrade(f.grade), filteredExams=exams.filter(e=>!f.category||examCategory(e)===f.category),subs=[...new Set(filteredExams.map(e=>e.subject))], years=[...new Set(filteredExams.filter(e=>!f.subject||e.subject===f.subject).map(e=>e.year))], choices=filteredExams.filter(e=>(!f.subject||e.subject===f.subject)&&(!f.year||String(e.year)===String(f.year))), ex=exams.find(e=>e.id===f.examId);
