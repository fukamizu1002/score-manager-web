...
- 結果入力の学年は小6・中3のみ
- 学年選択時に対象科目を自動表示
- 復習状況は復習ノート期限・提出日から自動判定
- 登録済み結果を後日編集し、復習ノート提出日を追記可能
- 確定済みの都立高・都立中過去問476件を重複なしで一括登録
...
## 定番過去問の初回登録
各校舎でログインし、「過去問」→「確定済み過去問を一括登録」を1回押します。同じ種類・学校・年度・科目の過去問は重複登録されません。校舎分離を保つため、StudyShareとena高田馬場でそれぞれ実行します。
